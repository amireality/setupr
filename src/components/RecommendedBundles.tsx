...
const BundleSkeleton = ({ gradient, title, subtitle }: { gradient: string; title: string; subtitle: string }) => (
  <div className={cn(
    ...
    <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-2">
      <span className="font-display text-base md:text-xl font-bold text-foreground">{title}</span>
...
const RecommendedBundles = () => {
  ...
  // Bento span patterns for asymmetric layout
  const spanClasses = [
    "col-span-2 md:col-span-2 row-span-1", // starter - wide
    "col-span-2 md:col-span-1 row-span-1 md:row-span-2", // presence - tall on desktop
    "col-span-2 md:col-span-1 row-span-1", // legal-only - small
  ];
  ...
  return (
    <section className="py-16 md:py-28 relative">
      ...
      <div className="container px-4 md:px-6 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 auto-rows-[minmax(180px,auto)] gap-3 md:gap-4 max-w-5xl mx-auto">
          {bundles.map((bundle, index) => {
            ...
            return (
              <motion.div
                ...
                className={spanClasses[index % 3]}
              >
                <Link
                  to={`/services?bundle=${bundle.bundle_id}`}
                  className={cn(
                    "block rounded-2xl group/bento transition-all duration-300 p-3 md:p-4",
                ...
                >
                  ...
                  <div className="group-hover/bento:translate-x-2 transition duration-200 mt-3 md:mt-4 flex-1 flex flex-col">
                    <div className="flex items-start gap-2 md:gap-3 mb-2 md:mb-3">
                      <div className={cn(
                        "w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center shrink-0 transition-all duration-300",
                    ...
                    <div className="flex items-center justify-between mt-auto pt-2 md:pt-3 border-t border-border/20">
                      ...
                      <div className="flex items-center gap-1 text-primary text-xs md:text-sm font-medium">
                        View <ArrowRight className="w-3 h-3 md:w-4 md:h-4 group-hover/bento:translate-x-1 transition-transform" />
                      </div>
                    </div>
                  </div>
                </Link>
